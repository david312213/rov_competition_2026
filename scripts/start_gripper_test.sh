...
用法：
  ...
  ./scripts/start_gripper_test.sh dual_dof [--action open|close|raise|reset] [--pwm N --from-pwm N]
...
PROFILE="$1"
shift
if [[ "${PROFILE}" != "dalian" && "${PROFILE}" != "rst" && "${PROFILE}" != "dual_dof" ]]; then
  echo "档案只能是 dalian、rst 或 dual_dof。" >&2
  exit 2
fi
