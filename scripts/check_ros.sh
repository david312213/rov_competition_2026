...
OFFICIAL_EXECUTABLES="$(ros2 pkg executables ros2_topic_forwarding)"
if ! grep -q " topic_forwarding$" <<<"${OFFICIAL_EXECUTABLES}"; then
  echo "缺少官方ROS转发命令: ros2_topic_forwarding topic_forwarding" >&2
  exit 1
fi

for REQUIRED_SCRIPT in start_blind_grab.sh start_blind_grab_team1.sh start_blind_grab_team2.sh start_official_data_only.sh start_official_data_only_team1.sh start_official_data_only_team2.sh set_official_team.sh check_official_ros.sh apply_blind_grab_power_08.sh prepare_blind_grab_old_pc.sh; do
