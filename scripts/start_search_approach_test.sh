...
CLUSTER_TARGETS_CONFIG="${PROJECT_DIR}/ros2_ws/src/rov_competition/config/cluster_collection_targets.yaml"
...
usage() {
  printf '%s\n' \
    '用法：./scripts/start_search_approach_test.sh [--workflow auto_approach|manual_grasp_calibration|cluster_approach|cluster_collection]' \
    '' \
    'auto_approach：自动下潜、扫描、对准、接近，然后回收。' \
    'manual_grasp_calibration：自动对准后停车，进入 WASD 抓取位置标定。' \
    'cluster_approach：触底、离底搜索、群体靠近，到接近判据后安全停车。' \
...
if [[ "${WORKFLOW}" != "auto_approach" && "${WORKFLOW}" != "manual_grasp_calibration" && "${WORKFLOW}" != "cluster_approach" && "${WORKFLOW}" != "cluster_collection" ]]; then
  echo "未知工作流：${WORKFLOW}" >&2
  exit 2
fi

if [[ "${WORKFLOW}" == "cluster_approach" || "${WORKFLOW}" == "cluster_collection" ]]; then
  ACTIVE_TARGETS_CONFIG="${CLUSTER_TARGETS_CONFIG}"
else
  ACTIVE_TARGETS_CONFIG="${TARGETS_CONFIG}"
fi
...
for required in \
  ...
  "${AUTONOMY_TEMPLATE}" "${ACTIVE_TARGETS_CONFIG}" "${SEARCH_CONFIG}" "${CLUSTER_CONFIG}"; do
...
if [[ "${WORKFLOW}" == "manual_grasp_calibration" ]]; then
  ...
  MODE_TITLE="抓取位置标定"
elif [[ "${WORKFLOW}" == "cluster_approach" ]]; then
  SESSION_ROOT="${PROJECT_DIR}/output/cluster_approach_tests"
  MODE_TITLE="无机械爪群体搜索—接近"
...
ros2 launch rov_competition perception_only.launch.py \
  ...
  targets_config:="${ACTIVE_TARGETS_CONFIG}" \
...
set +e
if [[ "${WORKFLOW}" == "cluster_collection" || "${WORKFLOW}" == "cluster_approach" ]]; then
  CLUSTER_RUNTIME_ARGS=()
  if [[ "${WORKFLOW}" == "cluster_approach" ]]; then
    CLUSTER_RUNTIME_ARGS+=(--stop-after-approach)
  fi
