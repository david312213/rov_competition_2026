...
一队使用 `./scripts/start_blind_grab_team1.sh`（官方端口40198），二队 `ddhyzx2` 使用 `./scripts/start_blind_grab_team2.sh`（官方端口40197）。全部运动功率已设为1.0：启动后以-1.0下潜，抓取及蛇形前进为1.0，横移和转向按路线方向使用±1.0，投放后以+1.0上潜。
