...
一队使用 `./scripts/start_blind_grab_team1.sh`（官方端口40198），二队 `ddhyzx2` 使用 `./scripts/start_blind_grab_team2.sh`（官方端口40197）。全部运动功率已设为0.8：启动后以-0.8下潜，抓取及蛇形前进为0.8，横移和转向按路线方向使用±0.8，投放后以+0.8上潜。
