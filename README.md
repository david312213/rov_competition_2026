...
一队使用 `./scripts/start_blind_grab_team1.sh`（官方端口40198），二队 `ddhyzx2` 使用 `./scripts/start_blind_grab_team2.sh`（官方端口40197）。升沉功率已设为0.8：启动后以-0.8立即下潜，首次压力深度稳定3秒或10秒兜底后抓取；随后永久执行“抓取投放→以+0.8上潜2秒→四段蛇形前进→以-0.8定时下潜”。
