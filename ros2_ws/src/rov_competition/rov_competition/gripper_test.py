...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("profile", choices=("dalian", "rst", "dual_dof"))
    parser.add_argument("--action", choices=("open", "close", "raise", "reset"))
    parser.add_argument("--pwm", type=int, help="只发一个明确 PWM；仅供 dual_dof 单步调整")
    parser.add_argument("--from-pwm", type=int, help="操作员核对的当前位置 PWM；软件没有位置反馈")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if (args.action is not None or args.pwm is not None or args.from_pwm is not None) and args.profile != "dual_dof":
            raise GripperTestError("独立动作/单步调整仅用于 dual_dof；旧档案保留原测试流程")
        if args.pwm is not None:
            from .dual_gripper_calibration import single_step
            config = single_step(config, args.action, args.pwm, args.from_pwm)
        elif args.from_pwm is not None:
            raise GripperTestError("from-pwm 必须与 pwm 同时提供")
    ...
    try:
        ...
        if args.profile == "dual_dof":
            from .dual_gripper_calibration import run_bench, signature
            records, confirmed = run_bench(vehicle, config, confirmation=typed,
                extended_confirmation=extended_confirmation, selected_action=args.action, records=records)
            actual_opened, actual_closed = confirmed.get("open"), confirmed.get("close")
            extra = {"actual_raised": confirmed.get("raise"), "actual_reset": confirmed.get("reset"),
                "calibration_signature": signature(config.gripper),
                "single_step_pwm": args.pwm, "from_pwm": args.from_pwm,
                "position_feedback": False}
            passed = len(confirmed) == 4 and all(confirmed.values()) and args.pwm is None
            outcome = "passed" if passed else "not_calibrated"
            detail = "四动作人工标定通过" if passed else "单步/单动作记录或动作未通过，不能激活"
