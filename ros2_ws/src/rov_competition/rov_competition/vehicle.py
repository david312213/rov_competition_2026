...
class MavlinkVehicle:
    ...
    def __init__(
        ...
        *,
        ...
        self._gripper_ack_deadline: float | None = None
        self._gripper_ack_error: str | None = None
    ...
    def set_gripper(
        ...
        with self._lock:
            ...
            self._gripper_ack_error = None
            self._gripper_ack_deadline = current + 2.0 if self.config.gripper.profile == "dual_dof" else None
            ...
            if not self._gripper_pending_steps and self._gripper_ack_deadline is None:
                self._gripper_action = GripperAction.NONE
            elif self._gripper_ack_deadline is not None:
                self._gripper_next_step_at = current + self.config.gripper.step_interval_s
    ...
    def run_disarmed_gripper_test(
        ...
        *,
        ...
        if action not in {GripperAction.OPEN, GripperAction.CLOSE, GripperAction.RAISE, GripperAction.RESET}:
            raise VehicleError("机械爪候选测试只允许 open/close/raise/reset")
    ...
    def update_gripper(self, *, now: float | None = None) -> bool:
        ...
        with self._lock:
            if self._gripper_action == GripperAction.NONE:
                return False
            if self._gripper_ack_error is not None:
                reason = self._gripper_ack_error
                self._finish_gripper_sequence()
                raise VehicleError(reason)
            if self._gripper_ack_deadline is not None:
                if current >= self._gripper_ack_deadline:
                    self._finish_gripper_sequence()
                    raise VehicleError("舵机 COMMAND_ACK 超时；不重发")
                return False
        ...
        with self._lock:
            ...
            try:
                ...
                if self.config.gripper.profile == "dual_dof":
                    self._gripper_ack_deadline = current + 2.0
            except Exception as exc:
                self._finish_gripper_sequence()
                raise VehicleError(f"机械爪序列发送失败: {exc}") from exc
            if self._gripper_pending_steps or self._gripper_ack_deadline is not None:
    ...
    def _finish_gripper_sequence(self) -> None:
        ...
        self._gripper_ack_deadline = None
        self._gripper_ack_error = None
    ...
    def _apply_slew_limit(self, target: MotionCommand, now: float) -> MotionCommand:
        ...
        # Opt-in for cluster sessions: do not keep pushing upward after a
        # neutral request. Acceleration, downward output and other axes keep
        # the existing slew limit. Emergency stop remains an independent path.
        vertical = (
            0.0
            if self.config.upward_neutral_bypass_slew
            and previous.vertical > 0.0
            and target.vertical == 0.0
            else approach(previous.vertical, target.vertical)
        )
    ...
    def _update_telemetry(self, message: Any, now: float) -> None:
        ...
        message_type = message.get_type()
        if (message_type == "COMMAND_ACK" and int(getattr(message, "command", -1)) == MAV_CMD_DO_SET_SERVO
                and self._gripper_ack_deadline is not None):
            # One servo command in flight; only target-autopilot messages reach here.
            # MAVLink ACK has no request ID; do not allow any other servo writer concurrently.
            result = int(getattr(message, "result", -1))
            if result == MAV_RESULT_ACCEPTED:
                self._gripper_ack_deadline = None
            elif result != MAV_RESULT_IN_PROGRESS:
                self._gripper_ack_error = f"飞控拒绝舵机命令，ACK={result}"
