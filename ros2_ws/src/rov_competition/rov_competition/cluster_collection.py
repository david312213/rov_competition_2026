...
class DescentTrigger(str, Enum):
    ...
    BOTH = "both"
    TARGET_BOTTOM_LINE = "target_bottom_line"
...
class ClusterCollectionState(str, Enum):
    ...
    RETURNING = "returning"
    RAISING_CLAW = "raising_claw"
    RELEASING_CATCH = "releasing_catch"
    WAITING_DROP = "waiting_drop"
    CLOSING_FOR_RESET = "closing_for_reset"
    RESETTING_CLAW = "resetting_claw"
    REACQUIRING_GROUP = "reacquiring_group"
    COMPLETE = "complete"
    ABORTED = "aborted"
...
class ClusterCollectionConfig:
    ...
    # None disables only the absolute-depth cutoff, not telemetry/timeout checks.
    maximum_operation_depth_m: float | None = 20.0
    maximum_depth_step_m: float = 0.50
    depth_recovery_stable_s: float = 0.50
    depth_anomaly_timeout_s: float = 2.0
    ...
    bottom_clearance_up_command: float = 0.35
    ...
    target_bottom_line_y_ratio: float = 0.80
    target_near_bottom_y_ratio: float = 0.65
    grasp_forward_command: float = 0.10
    grasp_descent_command: float = 0.35
    grasp_descent_timeout_s: float = 10.0
    ...
    def __post_init__(self) -> None:
        positive = (
            self.grasp_forward_command,
            self.grasp_descent_command,
            self.grasp_descent_timeout_s,
            self.maximum_depth_step_m,
            self.depth_recovery_stable_s,
            self.depth_anomaly_timeout_s,
        ...
        if self.maximum_operation_depth_m is not None and (
            isinstance(self.maximum_operation_depth_m, bool)
            or not math.isfinite(self.maximum_operation_depth_m)
            or self.maximum_operation_depth_m <= 0.0
        ):
            raise ClusterCollectionError("作业深度上限必须是正有限数或 null")
        if self.depth_recovery_stable_s >= self.depth_anomaly_timeout_s:
            raise ClusterCollectionError("深度恢复确认时间必须小于异常中止时间")
        ...
        if not 0.0 < self.target_near_bottom_y_ratio < self.target_bottom_line_y_ratio < 1.0:
            raise ClusterCollectionError("目标下边缘必须满足 0 < near < trigger < 1")
...
class ClusterCollectionDecision:
    ...
    target_bottom_ratio: float | None = None
    descent_vertical_only: bool = False


class ClusterCollectionMission:
    """触底、离底搜索、群体靠近和可选的三次盲抓状态机。"""

    def __init__(
        self, config: ClusterCollectionConfig, *, stop_after_approach: bool = False
    ) -> None:
        self.config = config
        # 无机械爪测试只复用到群体接近判据；正式盲抓保持默认行为。
        self.stop_after_approach = bool(stop_after_approach)
        ...
        self.last_plausible_depth_m: float | None = None
        self.depth_anomaly_started_at: float | None = None
        self.depth_recovery_started_at: float | None = None
    ...
    def start(
        ...
        *,
        ...
        self.last_plausible_depth_m = float(observation.depth_m)
        self.depth_anomaly_started_at = None
        self.depth_recovery_started_at = None
    ...
    def step(
        ...
        # A rejected sample must not stop the probe's wall-clock timeout.
        if self.state in {
            ClusterCollectionState.PROBING_BOTTOM,
            ClusterCollectionState.GRASP_DESCENDING,
        } and now - self.state_started_at >= self.config.bottom_probe_timeout_s:
            return self.abort("下潜触底等待超时", observation)
        depth_decision = self._check_depth_sample(observation, now)
        ...
        if (
            self.config.maximum_operation_depth_m is not None
            and observation.depth_m >= self.config.maximum_operation_depth_m
        ):
            return self.abort(
        ...
        extension = self._step_extension(observation, now, new_frame)
    ...
    def _step_extension(self, observation, now, new_frame):
        """Optional states run only after common telemetry/depth/frame checks."""
        return None
    ...
    def _check_depth_sample(
        self, observation: MissionObservation, now: float
    ) -> ClusterCollectionDecision | None:
        """Reject discontinuities without treating a frozen/rejected value as bottom.

        Keep the last plausible depth as the reference: repeated bad samples must
        not establish a new baseline. Recovery is automatic, but requires a short
        uninterrupted run of plausible observations; prolonged doubt aborts.
        These checks detect gross jumps, not every pressure-sensor failure.
        """

        depth = float(observation.depth_m)
        reference = self.last_plausible_depth_m
        if reference is None:
            return self.abort("可信深度基准缺失", observation)
        suspicious = abs(depth - reference) > self.config.maximum_depth_step_m
        if suspicious:
            if self.depth_anomaly_started_at is None:
                self.depth_anomaly_started_at = float(now)
            self.depth_recovery_started_at = None
            ...
            self.last_plausible_depth_m = depth
            if self.depth_anomaly_started_at is None:
                return None
            if self.depth_recovery_started_at is None:
                self.depth_recovery_started_at = float(now)

        assert self.depth_anomaly_started_at is not None
        if now - self.depth_anomaly_started_at >= self.config.depth_anomaly_timeout_s:
            return self.abort("深度跳变后未能及时恢复可信读数，禁止继续下潜", observation)
        if (
            self.depth_recovery_started_at is not None
            and now - self.depth_recovery_started_at
            >= self.config.depth_recovery_stable_s
        ):
            self.depth_anomaly_started_at = None
            self.depth_recovery_started_at = None
            # Do not count time spent holding against bottom-confirmation time.
            self.depth_history.clear()
            return None
        return self._decision(
            MotionCommand.neutral(),
            observation,
            f"深度跳变复核：可信基准={reference:.2f}m，读数={depth:.2f}m；"
            "运动回中，等待读数自动恢复；不计入触底稳定时间",
        )
    ...
    def _step_clearing_bottom(
        ...
        remaining_m = observation.depth_m - self.target_depth_m
        ...
        return self._decision(
            ...
            f"剩余={max(0.0, remaining_m):.2f}m，power=+{clearance.vertical_command:.2f}",
        )
    ...
    def _step_scanning(
        ...
        if self.scan_progress_deg >= self.config.scan_angle_deg - self.config.scan_tolerance_deg:
            ...
            return self._decision(
                MotionCommand.neutral(), observation,
                f"360° 未确认至少 {self.config.minimum_cluster_count} 个扇贝的群体，"
                f"准备前进 {self.config.search_advance_duration_s:g}s",
            )
        return self._decision(
    ...
    def _step_approaching(
        ...
        if (
            ...
            if self.stop_after_approach:
                self._enter(ClusterCollectionState.COMPLETE, observation, now)
                return self._decision(
                    MotionCommand.neutral(),
                    observation,
                    "群体接近完成；无机械爪模式已回中并进入安全停止",
                )
            self.grasp_attempts_completed = 0
...
STATE_DISPLAY_NAMES = {
    ClusterCollectionState.RAISING_CLAW: "底部抬爪（指令与标定等待）",
    ClusterCollectionState.RELEASING_CATCH: "顶部张爪投放",
    ClusterCollectionState.WAITING_DROP: "等待落料",
    ClusterCollectionState.CLOSING_FOR_RESET: "投放后闭爪",
    ClusterCollectionState.RESETTING_CLAW: "抬爪复位",
    ClusterCollectionState.REACQUIRING_GROUP: "重新选择当前群体目标",
