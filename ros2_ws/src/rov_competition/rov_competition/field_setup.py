...
GRIPPER_ACTIVATION_CONFIRMATIONS = {
    ...
    "dual_dof": "ACTIVATE DUAL DOF GRIPPER",
}
...
def validate_gripper_result(
    ...
    *,
    ...
    required_actions = {"open", "close", "raise", "reset"} if profile == "dual_dof" else {"open", "close"}
    if not rows or {row.get("action") for row in rows} != required_actions:
    ...
    if profile == "dual_dof":
        ...
        if data.get("actual_raised") is not True or data.get("actual_reset") is not True or data.get("single_step_pwm") is not None:
            raise FieldSetupError("抬爪/复位未经人工标定；单步记录不能激活")
        resolved = result.parent / "resolved_gripper_test.yaml"
        if not _inside(resolved.resolve(), result.parent.resolve()):
            raise FieldSetupError("标定配置越界")
        calibrated = load_robot_config(resolved).gripper
        if calibrated.profile != "dual_dof" or signature(calibrated) != data.get("calibration_signature"):
            raise FieldSetupError("标定参数已变化或签名缺失")
        expected = [(action, channel, pwm) for action in ("open", "close", "raise", "reset")
                    for step in calibrated.steps_for(action) for channel, pwm in step]
        actual = [(r["action"], int(r["output_channel"]), int(r["pwm"])) for r in rows]
        if actual != expected:
            raise FieldSetupError("四动作顺序/PWM记录与标定参数不一致")
...
def activate_gripper(
    ...
    *,
    ...
    if profile == "dual_dof":
        from .config import load_robot_config, _load_gripper_config
        from .dual_gripper_calibration import signature
        tested = load_robot_config(evidence.result_path.parent / "resolved_gripper_test.yaml").gripper
        candidate = _load_gripper_config({"active_profile": profile, "profiles": profiles})
        if signature(tested) != signature(candidate):
            raise FieldSetupError("待激活参数与台架实测参数不同，拒绝激活")
...
def verify_gripper_activation(project_directory: str | Path) -> tuple[bool, str]:
    ...
    try:
        ...
        if profile == "dual_dof":
            ...
            from .dual_gripper_calibration import signature
            tested = load_robot_config(evidence.result_path.parent / "resolved_gripper_test.yaml").gripper
            current = load_robot_config(paths.robot_config).gripper
            if signature(tested) != signature(current):
                raise FieldSetupError("当前通道/PWM/动作时间与标定证据不一致")
...
def write_runtime_robot_config(
    ...
    *,
    ...
    if not isinstance(upward_neutral_bypass_slew, bool):
        raise FieldSetupError("upward_neutral_bypass_slew 必须是布尔值")
    paths = field_paths(project_directory)
    robot = _read_yaml(paths.robot_config, "实艇配置")
    safety = _mapping(robot.get("safety"), "safety")
    if mode == "auto":
    ...
    robot["safety"] = safety
    if upward_neutral_bypass_slew:
        control = _mapping(robot.get("control"), "control")
        control["upward_neutral_bypass_slew"] = True
    ...
    if upward_neutral_bypass_slew and not loaded.upward_neutral_bypass_slew:
        raise FieldSetupError("群体会话上浮回中配置自检失败")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    latest.add_argument("--profile", required=True, choices=("dalian", "rst", "dual_dof"))
    activate = gripper_actions.add_parser("activate")
    activate.add_argument("--profile", required=True, choices=("dalian", "rst", "dual_dof"))
    ...
    runtime.add_argument(
        "--upward-neutral-bypass-slew", action="store_true",
        help="仅本会话：向上输出回中时立即归零，其余斜率限制不变",
    )
    ...
    try:
        ...
        if args.group == "runtime-config":
            enabled, reason = write_runtime_robot_config(
                ...
                upward_neutral_bypass_slew=args.upward_neutral_bypass_slew,
            )
            print(
                f"会话配置已生成；机械爪 {'ENABLED' if enabled else 'DISABLED'}：{reason}"
            )
            if args.upward_neutral_bypass_slew:
                print("本会话上浮指令回中立即归零；加力限速、其他运动轴和失控保护保持不变。")
            return 0
