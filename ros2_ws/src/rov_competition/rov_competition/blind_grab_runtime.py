...
from .blind_grab import BlindGrabMission, DepthSample, DetectionCount
    ...
    def _on_message(self, message: Any) -> None:
        ...
        try:
            ...
            if now >= self._next_callback_report_at:
                ...
                self.report(f"[盲抓检测] 无法解析本帧: {exc}；运动流程不变")
        ...
        while not self.stop_event.is_set():
            ...
            try:
                ...
                if not self.stop_event.is_set():
                    self.report("[盲抓检测] ROS 检测上下文已关闭，稍后重新连接；运动流程不变")
            except Exception as exc:
                self.report(f"[盲抓检测] {type(exc).__name__}: {exc}；运动流程不变，稍后重试")
            finally:
...
def run_control_loop(
    mission: BlindGrabMission,
    detection_source: Any,
    depth_source: Callable[[], Any],
    output: Any,
    stop_event: Any,
    *, clock: Callable[[], float] = time.monotonic,
    report: Callable[[str], None] = say,
) -> None:
    """可注入时钟、观测源和输出的20Hz主循环；不会等待辅助进程。"""
    next_report_at = 0.0
    next_detection_error_at = 0.0
    next_depth_error_at = 0.0
    next_output_error_at = 0.0
    previous_key = None
    while not stop_event.is_set():
        now = clock()
        box_count = 0
        try:
            frame = detection_source.latest()
            if frame is not None:
                box_count = int(frame.count)
        except Exception as exc:
            if now >= next_detection_error_at:
                report(f"[盲抓检测] {exc}；只影响框数显示，运动流程不变")
                next_detection_error_at = now + 2.0

        depth = None
        try:
            snapshot = depth_source()
            if snapshot.depth_m is not None and snapshot.depth_at is not None:
                depth = DepthSample(float(snapshot.depth_at), float(snapshot.depth_m))
        except Exception as exc:
            if now >= next_depth_error_at:
                report(
                    f"[盲抓深度] {exc}；首次沉底按"
                    f"{mission.config.initial_fallback_s:g}秒兜底继续"
                )
                next_depth_error_at = now + 2.0

        decision = mission.step(now, depth)
        if stop_event.is_set():
            break
        try:
            output.set_desired(decision.motion, decision.servos)
        except Exception as exc:
            if now >= next_output_error_at:
                report(f"[盲抓输出] {exc}；下个控制周期继续")
                next_output_error_at = now + 2.0
        key = (
            decision.state,
            decision.phase,
            decision.bottom_source,
            decision.lane_index,
            decision.segment_in_lane,
            decision.completed_cycles,
        )
        if now >= next_report_at or key != previous_key:
            next_report_at, previous_key = now + 1.0, key
            depth_text = "none" if decision.depth_m is None else f"{decision.depth_m:.2f}m"
            report(
                f"[盲抓] state={decision.state.value} phase={decision.phase} "
                f"boxes={box_count} visual_control=false depth={depth_text} "
                f"bottom={decision.bottom_source} stable={decision.bottom_stable_s:.2f}s "
                f"lane={decision.lane_index + 1} "
                f"segment={decision.segment_in_lane}/{mission.config.route_steps_per_lane} "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="启动即沉底，随后永久执行抓取、上潜、蛇形移动和下潜；视觉只显示，Ctrl+C关闭。"
    )
