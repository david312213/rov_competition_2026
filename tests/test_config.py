...
def test_upward_neutral_bypass_is_opt_in(tmp_path: Path) -> None:
    assert load_robot_config(ROBOT_EXAMPLE).upward_neutral_bypass_slew is False
    data = yaml.safe_load(ROBOT_EXAMPLE.read_text(encoding="utf-8"))
    data["control"]["upward_neutral_bypass_slew"] = True
    path = tmp_path / "robot.yaml"
    path.write_text(yaml.safe_dump(data), encoding="utf-8")
    assert load_robot_config(path).upward_neutral_bypass_slew is True


@pytest.mark.parametrize("invalid", ("true", "false", 0, 1, None))
def test_upward_neutral_bypass_rejects_non_boolean(tmp_path: Path, invalid) -> None:
    original = load_robot_config(ROBOT_EXAMPLE)
    with pytest.raises(ConfigurationError):
        replace(original, upward_neutral_bypass_slew=invalid)
    data = yaml.safe_load(ROBOT_EXAMPLE.read_text(encoding="utf-8"))
    data["control"]["upward_neutral_bypass_slew"] = invalid
    path = tmp_path / "robot.yaml"
    path.write_text(yaml.safe_dump(data), encoding="utf-8")
    with pytest.raises(ConfigurationError):
...
def test_autonomy_model_path_and_hash_are_stable() -> None:
    ...
    assert config.detector.expected_sha256 == (
        "787b22100b6b9ee3c0b5aefce434a194c686b647a0c182beb4add38a0cc8fe2b"
    )
...
def test_live_readiness_lists_every_uncalibrated_gate() -> None:
    ...
    errors = mission.readiness_errors(("scallop", "snail"))
    assert len(errors) == 7
