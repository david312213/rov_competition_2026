...
def test_template_contains_confirmed_vehicle_arm_and_gripper_setpoints():
    ...
    assert actions["open_gripper"]["outputs"] == [{"output_channel": 11, "pwm": 1800}]
    assert actions["close_gripper"]["outputs"] == [{"output_channel": 11, "pwm": 900}]
    assert actions["arm_to_basket"]["outputs"] == [{"output_channel": 10, "pwm": 1800}]
    assert actions["arm_to_grasp"]["outputs"] == [{"output_channel": 10, "pwm": 900}]
