...
def test_vehicle_template_contains_permanent_dive_route_and_action_timing():
    ...
    assert mission.initial_descent_command == pytest.approx(-1.0)
    ...
    assert mission.ascent_command == pytest.approx(1.0)
    assert mission.ascent_duration_s == 2
    assert mission.repeat_descent_command == pytest.approx(-1.0)
    assert mission.repeat_descent_duration_s == 5
    assert mission.route_forward_command == pytest.approx(1.0)
    assert mission.route_step_duration_s == 5
    assert mission.route_steps_per_lane == 4
    assert mission.shift_command == pytest.approx(1.0)
    assert mission.shift_duration_s == 4.5
    assert mission.turn_command == pytest.approx(1.0)
    assert mission.turn_duration_s == 11.5
    assert mission.grab_forward_command == pytest.approx(1.0)
...
def test_old_permission_flags_and_visual_trigger_values_do_not_gate_mission(tmp_path):
    ...
    assert loaded.mission.route_forward_command == 1.0
...
def test_legacy_local_config_without_new_sections_gets_new_defaults(tmp_path):
    ...
    assert mission.route_steps_per_lane == 4
    assert mission.route_step_duration_s == 5
    assert mission.route_forward_command == 1.0
    assert mission.shift_command == 1.0
    assert mission.turn_command == 1.0
    assert mission.grab_forward_command == 1.0
