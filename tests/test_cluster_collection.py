...
BASE = load_cluster_collection_config(PACKAGE / "config/cluster_collection.yaml")
...
def test_default_config_matches_field_plan() -> None:
    assert BASE.minimum_cluster_count == 6
...
def test_five_scallops_never_trigger_but_six_trigger_on_three_of_five() -> None:
    mission = scanning_mission()
    for frame_id in range(1, 7):
        decision = mission.step(
            observation(frame_id, detections=scallop_group(5)), frame_id * 0.1
        )
        assert decision.state == ClusterCollectionState.SCANNING
    for frame_id in range(7, 10):
        decision = mission.step(
            observation(frame_id, detections=scallop_group(6)), frame_id * 0.1
        )
    assert decision.state == ClusterCollectionState.ALIGNING
    assert decision.cluster is not None and decision.cluster.count == 6
