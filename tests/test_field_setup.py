...
from rov_competition.config import load_robot_config
...
def test_runtime_cli_upward_neutral_is_session_only_and_preserves_safety(tmp_path: Path, enabled) -> None:
    project = _make_project(tmp_path)
    source = field_paths(project).robot_config
    original_bytes = source.read_bytes()
    original = load_robot_config(source)
    destination = project / "output/session/resolved_robot.yaml"
    args = ["--project-dir", str(project), "runtime-config", "--mode", "auto", "--output", str(destination)]
    if enabled:
        args.append("--upward-neutral-bypass-slew")
    assert main(args) == 0
    loaded = load_robot_config(destination)
    assert loaded.upward_neutral_bypass_slew is enabled
    assert loaded.allow_gripper_actuation is False
    assert source.read_bytes() == original_bytes
    # No arming, watchdog, timing or other robot setting is changed.
    before = yaml.safe_load(original_bytes)
    after = yaml.safe_load(destination.read_text(encoding="utf-8"))
    before["safety"]["allow_gripper_actuation"] = False
    if enabled:
        before["control"]["upward_neutral_bypass_slew"] = True
    assert after == before
    assert loaded.maximum_command_age_s == original.maximum_command_age_s == pytest.approx(0.5)
...
def test_weight_inspection_requires_cuda_and_all_grasp_targets(tmp_path: Path) -> None:
    ...
    assert inspected.class_names[: len(TARGETS)] == TARGETS
    ...
    with pytest.raises(FieldSetupError, match="scallop"):
        inspect_weight(
            source,
    ...
    class ScallopOnlyYolo(_FakeYolo):
        names = {0: "scallop"}

    assert inspect_weight(
        source,
        yolo_factory=ScallopOnlyYolo,
        torch_module=_FakeTorch(),
        numpy_module=_FakeNumpy(),
    ).class_names == ("scallop",)
